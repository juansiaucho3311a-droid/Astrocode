...
class Game {
  ...
  /* --- Pausa / reanuda la partida (tecla ESC o botón de pausa) --- */
  togglePause() {
  ...
  resumeGame() {
    ...
    this.state = "playing";
    this.pauseScreen.classList.add("hidden");
    // Evita un salto brusco de dt tras reanudar
    this.lastTimestamp = 0;
  }

  /* --- Vuelve a la pantalla principal desde la pausa --- */
  goToMenu() {
    this.state = "menu";
    this.pauseScreen.classList.add("hidden");
    this.pauseBtn.classList.add("hidden");
  ...
  /* ------------------------------------------------------------
     ...
     ------------------------------------------------------------ */
  handleKeyDown(e) {
    // ESC alterna pausa/reanudación, tanto jugando como en pausa
    if (e.key === "Escape") {
      e.preventDefault();
      this.togglePause();
      return;
